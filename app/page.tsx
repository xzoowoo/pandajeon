...
interface Comment {
  id: number; author: string; content: string; time: string; likes: number; is_liked: boolean; replies?: Comment[];
}

interface Post {
  id: number; sub: string; title: string; content: string; images: string[]; author: string; likes: number; is_liked: boolean; comments: Comment[]; created_at: string;
}

export default function PandajeonFullOptimized() {
  ...
  const handlePostSubmit = async () => {
    ...
    try {
      ...
      for (const file of selectedFiles) {
        ...
        const { data } = await supabase.storage.from('photos').upload(fileName, compressedFile);
        if (data) {
  ...
  return (
    <div className="min-h-screen bg-[#DAE0E6] font-sans pb-10 text-[#1A1A1B]">
      {fullImage && <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-4 cursor-zoom-out" onClick={() => setFullImage(null)}><img src={fullImage} className="max-w-full max-h-full object-contain rounded" alt="" /></div>}
      ...
      <nav className="sticky top-0 bg-white border-b h-12 flex items-center justify-center px-5 z-50 shadow-sm">
        ...
        <div className="absolute right-5 text-xs font-bold text-gray-700 bg-gray-50 px-3 py-1 rounded-full border">{currentUser}</div>
      </nav>
      ...
      <div className="max-w-[800px] mx-auto py-5 flex gap-6 px-4">
        <aside className="w-48 hidden lg:block sticky top-20 h-fit">
          <div className="bg-white rounded p-3 border shadow-sm">
            ...
            {Array.from(new Set(["전체", "자유", ...posts.map(p => p.sub)])).map(sub => (
        ...
        <main className="flex-1 space-y-4">
          <div className="bg-white p-4 rounded border shadow-sm space-y-3">
            ...
            <div className="flex justify-between items-center pt-2 border-t">
              ...
              <button onClick={handlePostSubmit} disabled={isUploading} className="bg-blue-600 text-white px-6 py-1.5 rounded-full font-bold text-sm flex items-center gap-2 transition active:scale-95">
                    ...
                    <div className="bg-gray-50 border-t p-4 space-y-4" onClick={(e) => e.stopPropagation()}>
                      ...
                      {post.comments?.map(comment => (
                        <div key={comment.id} className="space-y-2">
                          ...
                          <div className="flex items-center gap-3 text-[10px] font-bold text-gray-400"><button onClick={() => toggleCommentLike(post.id, post.comments, comment.id)} className={comment.is_liked ? 'text-red-500' : ''}><Heart size={12} fill={comment.is_liked ? "currentColor" : "none"}/> {comment.likes || 0}</button><button onClick={() => setReplyingTo(replyingTo === comment.id ? null : comment.id)} className="hover:text-blue-500">답글 달기</button></div></div></div>
                          {comment.replies?.map(reply => (
                            <div key={reply.id} className="flex gap-2 pl-8"><CornerDownRight size={14} className="text-gray-300 mt-1 flex-shrink-0"/><div className="flex-1 bg-gray-100 p-2 rounded-lg border border-gray-200"><div className="text-[10px] font-bold mb-0.5">{reply.author}</div><p className="text-xs text-gray-700 mb-2">{reply.content}</p><button onClick={() => toggleCommentLike(post.id, post.comments, reply.id)} className={reply.is_liked ? 'text-red-500' : ''}><Heart size={10} fill={reply.is_liked ? "currentColor" : "none"}/> {reply.likes || 0}</button></div></div>
                          ))}
